...
<div className="bg-white rounded-md shadow-md text-gray-700 p-4 divide-y">
    {/* User profile image or placeholder icon */}
    <div className="flex items-center space-x-4">
        {session.user.image ? (
            ...
            <AiOutlineUser
                size={20}
                className="h-10 w-10 bg-red-500 text-white rounded-full p-2 cursor-pointer hover:bg-red-600"
            />
        )}
        ...
        <form className="flex flex-1">
            <input
                ...
                placeholder={`What's on your mind, ${session?.user.name}?`}
            />
            <button hidden onClick={handleSubmit} className="ml-2 px-4 py-2 bg-red-500 text-white rounded-full hover:bg-red-600">
                Post
            </button>
        </form>
    </div>
    ...
    {imageToPost && (
        <div
            onClick={removeImage}
            className="flex items-center px-4 py-2 space-x-4 filter hover:brightness-110 transition duration-150 cursor-pointer mt-4"
        >
            <img src={imageToPost} className="h-16 object-cover rounded-md" />
            <RiDeleteBin6Line className="h-8 hover:text-red-500" />
        </div>
    )}
    ...
    <div className="flex justify-evenly py-4">
        <div
            onClick={handleClick}
            className="flex items-center p-2 space-x-2 flex-grow justify-center hover:cursor-pointer hover:bg-red-100 rounded-md"
        >
            <IoMdPhotos className="text-red-500" size={24} />
            <p className="font-semibold text-gray-700">Add Photo/Video</p>
            <input
                ref={hiddenFileInput}
                onChange={addImageToPost}
                type="file"
